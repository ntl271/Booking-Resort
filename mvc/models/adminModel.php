...
class adminModel extends connectDB{
    ...
    function checkTaiKhoan($username, $pass, $email , $sdt , $ten){
    ...
    function getTaiKhoan(){
        $sql  = "SELECT * FROM administrator LIMIT 10 ";
            $result  = $this->connect->query($sql);
            $taikhoan = array();
    ...
    function getOneTaiKhoan($username) {
        $sql  = "SELECT * FROM administrator WHERE username='$username'";
            $result  = $this->connect->query($sql);
            $one_taikhoan = array();
            if ($result->num_rows > 0) {
        ...
        return json_encode($one_taikhoan);
    }

    function add_taikhoan_model($username, $pass, $email , $sdt , $ten){
        $sql = "INSERT INTO `administrator`(`username`, `pass`, `email`, `sdt`, `ten`) 
        VALUES ('$username','$pass','$email' , '$sdt' ,'$ten' )";
        $result = $this->connect->query($sql);
        if($result){
            return true;
        } else{
    ...
    function update_taikhoan_model($username,$pass, $email , $sdt , $ten){
            $sql ="UPDATE `administrator` SET pass='$pass',email='$email',sdt='$sdt',ten='$ten' WHERE username='$username'";
            $result = $this->connect->query($sql);
            if($result){
                return true;
            } else{
    ...
    function delete_taikhoan($username){
